...
def deduplicate_chunks(chunks, overlap_threshold=0.5):
    """
    Remove duplicate or overlapping chunks that contain the same text.
    """
    if not chunks:
        return chunks
    ...
    deduplicated = []
    for chunk in chunks:
        # Check if this chunk's text is already covered by previous chunks
        is_duplicate = False
        ...
        for existing_chunk in deduplicated:
            ...
            if (chunk_text in existing_text or existing_text in chunk_text or 
                chunk_text == existing_text):
                is_duplicate = True
                break
...
async def whisper(file, response_format: str, **kwargs):
    ...
    if response_format == "text":
        ...
        # Deduplicate chunks to remove repeated text
        chunks = deduplicate_chunks(chunks)

        response = {
            "task": kwargs['generate_kwargs']['task'],
            #"language": "english",
            "duration": chunks[-1]['timestamp'][1] if chunks else 0,
        ...
        return JSONResponse(content=response, media_type="application/json", headers={"Content-Disposition": f"attachment; filename={filename_noext}_verbose.json"})
            ...
            return PlainTextResponse("\n".join([ f"{i}\n{srt_time(chunk['timestamp'][0])} --> {srt_time(chunk['timestamp'][1])}\n{chunk['text'].strip()}\n"
                for i, chunk in enumerate(chunks, 1) ]), media_type="text/srt; charset=utf-8", headers={"Content-Disposition": f"attachment; filename={filename_noext}.srt"})
            ...
            return PlainTextResponse("\n".join(["WEBVTT\n"] + [ f"{vtt_time(chunk['timestamp'][0])} --> {vtt_time(chunk['timestamp'][1])}\n{chunk['text'].strip()}\n"
                for chunk in chunks ]), media_type="text/vtt; charset=utf-8", headers={"Content-Disposition": f"attachment; filename={filename_noext}.vtt"})
...
#        kwargs["initial_prompt"] = prompt
    ...
    # Set temperature to 0 by default to reduce hallucinations
    if temperature is not None:
        kwargs['generate_kwargs']["temperature"] = temperature
        if temperature > 0:
            kwargs['generate_kwargs']['do_sample'] = True
    else:
        kwargs['generate_kwargs']["temperature"] = 0.0
...
if __name__ == "__main__":
    ...
    pipe = pipeline("automatic-speech-recognition", model=args.model, device=device, chunk_length_s=30, stride_length_s=5, torch_dtype=dtype)
    if args.preload:
        sys.exit(0)

    app.register_model('whisper-large-v3', args.model)
